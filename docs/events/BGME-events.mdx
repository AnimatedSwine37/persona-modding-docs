...
The [BGME Framework](https://gamebanana.com/mods/477399) provides basic event music editing functionality. It has some advantages over [manually editing the event files](./event-audio-editing-pmx.mdx), such as easier to read syntax and hot reload, which allows you to edit and test events in real time. However, it is incapable of doing more advanced edits at this time; for these, [directly editing the .PM2 and .PM3 files](./event-audio-editing-pmx.mdx) is necessary. Regardless of how you edit events, BGME's event information logging and hot reload features are very helpful in the making of event music mods.

Editing events with BGME is done using a convenient scripting language that can be used to quickly and easily edit event music calls. To use this, create a new folder in your mod directory called `BGME`. In this folder, create a new text file called `(script name).pme`, where `(script name)` can be replaced with a name of your choice. This can be done in the same file used for [custom battle music](../audio/battle-music.mdx) if desired.
...
![](/assets/images/events/r2_bgme_debug.png)
...
![](/assets/images/events/r2_bgme_hotreload.png)

Similarly, CRI FileSystem V2 Hook should also have Hot Reload enabled.

![](/assets/images/events/r2_crifs_hotreload.png)
...
Additional information on editing events can be found [here](event-audio-editing-pmx.mdx). It is also necessary to directly edit the .PM2 and .PM3 files as described there for advanced cases, such as but not limited to:
